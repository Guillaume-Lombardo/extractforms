...
- Python 3.13+
- `uv`
- Optional: Docker (if your project needs local service dependencies)
...
- Use typed enums for user-facing choices (`StrEnum`, `Flag`/`IntFlag`) when relevant.
- Keep docstrings in Google style with typed `Args`, `Returns`, and `Raises` where relevant.
- Prefer modular architecture and clear abstractions between layers.
